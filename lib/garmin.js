...
async function fetchDailySteps(tokens, startDate, endDate) {
  ...
  while (chunkStart <= end) {
    ...
    try {
      logger.info(`Fetching steps ${fromStr} to ${toStr}`);
      ...
      logger.error(`Failed to fetch steps for ${fromStr} to ${toStr}: ${err.message}`);
    }
  ...
  // Return updated tokens so the caller can persist them
  const updatedTokens = client.exportToken();
  logger.debug({ expires_at: updatedTokens.oauth2?.expires_at, last_update_date: updatedTokens.oauth2?.last_update_date }, 'exportToken oauth2');
  return { steps: results, updatedTokens };
}
