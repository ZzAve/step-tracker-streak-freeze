...
const { calculateStreak, applyIncrementalDays, extendStreakWithToday } = require('./streak');
...
function buildDayAnnotations(allSteps, freezesUsed, todayStr) {
  ...
  return allSteps
    ...
    .map((s) => ({
      date: s.date,
      status: s.goal_met ? 'hit' : freezeDates.has(s.date) ? 'freeze' : 'not_met',
    }));
}
...
async function fetchStepsAndStreak(userId) {
  ...
  if (streakRows.length > 0) {
    ...
    // computed but already in the past (yesterday or earlier).
    const unprocessedDays = lastProcessed
      ? allSteps.filter((s) => s.date > lastProcessed && s.date < todayStr)
      : null;

    if (lastProcessed && (!unprocessedDays || unprocessedDays.length === 0)) {
      ...
      const baseStreak = {
        ...
        day_annotations: buildDayAnnotations(allSteps, freezesUsed, todayStr),
      };
      return {
        allSteps,
        streak: extendStreakWithToday(baseStreak, allSteps, todayStr),
      };
    }

    if (unprocessedDays && unprocessedDays.length > 0) {
      ...
      if (safe) {
        ...
        streakResult = {
          ...result,
          day_annotations: buildDayAnnotations(allSteps, result.freezes_used, todayStr),
        };
      } else {
        // Fall back to full recalc.
        streakResult = calculateStreak(allSteps, null);
        const pastDays = allSteps.filter((s) => s.date < todayStr);
      ...
      return { allSteps, streak: extendStreakWithToday(streakResult, allSteps, todayStr) };
    }
  }

  // No persisted streak (or null last_processed_date with no unprocessed days).
  // Full recalc — sync will persist the result on the next trigger.
  const streak = calculateStreak(allSteps, null);
  return { allSteps, streak: extendStreakWithToday(streak, allSteps, todayStr) };
}
