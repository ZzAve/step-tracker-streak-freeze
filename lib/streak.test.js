...
// ---------------------------------------------------------------------------
// Scenario 13: a freeze covers exactly ONE missed day, so two banked freezes
// cover two CONSECUTIVE missed days and the streak survives.
// Pattern: 10 met (earn 2 freezes) + 2 miss (both auto-covered) → streak = 12
// ---------------------------------------------------------------------------
test('Scenario 13: 2 banked freezes cover 2 consecutive misses → streak continues', () => {
  // [MET x10, MISS, MISS] — 12 past days.
  const dates = pastDates(12, yesterday());
  const steps = [MET, MET, MET, MET, MET, MET, MET, MET, MET, MET, MISS, MISS];
  const days = makeDays(dates.map((date, i) => ({ date, steps: steps[i] })));
  const result = calculateStreak(days, null);
  // Both misses are bridged: 10 met + 2 freeze-covered = unbroken run of 12.
  assert.equal(result.current_streak, 12);
  // Each miss consumed one freeze; both freezes are now spent.
  assert.equal(result.freeze_count, 0);
  assert.equal(result.freezes_used.length, 2);
});

// ---------------------------------------------------------------------------
// Scenario 14: coverage is capped by freezes held (max 2). A THIRD consecutive
// missed day has no freeze left, so it breaks the streak.
// Pattern: 10 met (earn 2 freezes) + 3 miss → only 2 covered, streak = 0
// ---------------------------------------------------------------------------
test('Scenario 14: a 3rd consecutive miss exhausts both freezes and breaks the streak', () => {
  // [MET x10, MISS, MISS, MISS] — 13 past days.
  const dates = pastDates(13, yesterday());
  const steps = [MET, MET, MET, MET, MET, MET, MET, MET, MET, MET, MISS, MISS, MISS];
  const days = makeDays(dates.map((date, i) => ({ date, steps: steps[i] })));
  const result = calculateStreak(days, null);
  // The most-recent (3rd) miss is uncovered, so the current run is broken.
  assert.equal(result.current_streak, 0);
  // Only two misses were ever coverable; both freezes were spent doing so.
  assert.equal(result.freeze_count, 0);
  assert.equal(result.freezes_used.length, 2);
  // The pre-break run of 12 (10 met + 2 frozen) is retained as the record.
  assert.equal(result.longest_streak, 12);
});
