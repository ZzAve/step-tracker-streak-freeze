...
const { formatDateStr, buildDayAnnotations } = require('./steps.js');
...
test('buildDayAnnotations: a day counts as hit when steps meet the goal even if goal_met is false', () => {
  const allSteps = [
    { date: '2025-03-10', steps: 12000, goal_met: true },
    { date: '2025-03-11', steps: 10500, goal_met: false }, // goal_met stale, but steps >= STEP_GOAL
    { date: '2025-03-12', steps: 3000, goal_met: false },
  ];
  const annotations = buildDayAnnotations(allSteps, [], '2025-03-13');
  assert.deepEqual(annotations, [
    { date: '2025-03-10', status: 'hit' },
    { date: '2025-03-11', status: 'hit' },
    { date: '2025-03-12', status: 'not_met' },
  ]);
});

test('buildDayAnnotations: freeze dates and today exclusion', () => {
  const allSteps = [
    { date: '2025-03-10', steps: 2000, goal_met: false },
    { date: '2025-03-11', steps: 15000, goal_met: false }, // steps-based hit
    { date: '2025-03-12', steps: 1000, goal_met: false }, // today — excluded
  ];
  const annotations = buildDayAnnotations(allSteps, [{ date: '2025-03-10' }], '2025-03-12');
  assert.deepEqual(annotations, [
    { date: '2025-03-10', status: 'freeze' },
    { date: '2025-03-11', status: 'hit' },
  ]);
});
