...
module.exports = async (req, res) => {
  logger.debug({ method: req.method, url: req.url, query: req.query }, 'Request received');
  if (req.method !== 'GET') {
  ...
  try {
    ...
    logger.debug({ userId: user.id, last_synced_at: user.last_synced_at }, 'Checking sync');
    const didSync = await syncIfNeeded(user, { fatalOnMissingTokens: false, fatalOnSyncError: false });
    logger.debug('syncIfNeeded result: %s', didSync ? 'SYNCED' : 'SKIPPED (cooldown or no tokens)');

    // --- Fetch steps & calculate streak ---
    logger.debug('Fetching all step data for user %s', user.id);
    ...
    logger.debug('Found %d step records in DB', allSteps.length);

    const streak = calculateStreak(allSteps, null);
    logger.debug({ current: streak.current_streak, longest: streak.longest_streak, freezes: streak.freeze_count }, 'Streak result');
    ...
    // --- Compact response ---
    logger.debug({ streak: streak.current_streak, today_steps: todaySteps, next_milestone: nextMilestone }, 'Responding');
    res.status(200).json({
    ...
    logger.error(err, 'Error in /api/widget');
